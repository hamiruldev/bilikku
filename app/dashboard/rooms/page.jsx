...
import { useRooms, useCreateRoom, useDeleteRoom, useSublets, useRoomTypes, useSubletOptions } from '../../../hooks/useQueries';
...
export default function RoomsPage() {
  ...
  const { data: rooms = { items: [] }, isLoading: roomsLoading, error: roomsError } = useRooms();
  const { data: sublets = { items: [] } } = useSublets();
  const { data: roomTypes = [] } = useRoomTypes();
  const { data: subletOptions = [] } = useSubletOptions();
  ...
  if (roomsError) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-destructive">Error: {roomsError.message}</div>
      </div>
    );
  }
  ...
  const filteredRooms = rooms?.items?.filter((room) => {
    if (!room) return false;

    const matchesSearch =
      room.number?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      room.type?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      room.sublet_id?.name?.toLowerCase().includes(searchTerm.toLowerCase());

    const matchesStatus = !filters.status || room.status === filters.status;
    const matchesType = !filters.type || room.type === filters.type;
    const matchesSublet = !filters.sublet_id || room.house === filters.sublet_id;
